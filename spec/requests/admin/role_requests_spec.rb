RSpec.describe "Admin Role Requests", type: :request do
  ...
  before do
    allow(TradeTariffDevHub).to receive(:role_request_enabled?).and_return(true)
  ...
  describe "POST /admin/role_requests/:id/approve" do
    ...
    it "approves the role request and assigns the role", :aggregate_failures do
      expect { post approve_admin_role_request_path(role_request) }
        ...
        .and change { organisation.reload.has_role?("trade_tariff:full") }.from(false).to(true)
    end
    ...
    it "redirects to index with success message", :aggregate_failures do
      ...
      follow_redirect!
      expect(response.body).to include("Role trade_tariff:full has been assigned")
    end
  ...
  describe "POST /admin/role_requests/:id/reject" do
    let(:organisation) { create(:organisation, :trade_tariff_only) }
    let(:user) { create(:user, organisation: organisation) }
    ...
    it "rejects the role request and does not assign the role", :aggregate_failures do
      post reject_admin_role_request_path(role_request)
      expect(role_request.reload.status).to eq("rejected")
      expect(organisation.reload.has_role?("fpo:full")).to be false
    end
    ...
    it "redirects to index with success message", :aggregate_failures do
      ...
      expect(response).to redirect_to(admin_role_requests_path)
      follow_redirect!
      expect(response.body).to include("Role request for fpo:full from")
      expect(response.body).to include("has been rejected")
    end
    ...
    context "when role request not found" do
      it "redirects with error message", :aggregate_failures do
        post reject_admin_role_request_path("non-existent-id")
        expect(response).to redirect_to(admin_role_requests_path)
        follow_redirect!
        expect(response.body).to include("Role request not found")
      end
    ...
    context "when rejection fails" do
      before do
        allow(role_request).to receive(:reject!).and_raise(StandardError.new("Database error"))
        allow(RoleRequest).to receive(:find).and_return(role_request)
      end
      ...
      it "redirects with error message", :aggregate_failures do
        ...
        expect(response).to redirect_to(admin_role_requests_path)
        follow_redirect!
        expect(response.body).to include("There was an unexpected problem rejecting the role request")
      end
    end
  end
  ...
  describe "authorization" do
    ...
    context "when role_request_enabled? is false" do
      before do
        allow(TradeTariffDevHub).to receive(:role_request_enabled?).and_return(false)
      end

      it "redirects to root with flash message for index route", :aggregate_failures do
        get admin_role_requests_path
        expect(response).to redirect_to(root_path)
        follow_redirect!
      ...
      it "redirects to root with flash message for approve route", :aggregate_failures do
      ...
      it "redirects to root with flash message for reject route", :aggregate_failures do
        role_request = create(:role_request, status: "pending")
        post reject_admin_role_request_path(role_request)
        expect(response).to redirect_to(root_path)
        follow_redirect!
