...
private
  ...
  def limit_keys_per_organisation
    ...
    return unless enabled? # Skip validation when key is disabled
    ...
    existing_count = organisation.send(association_name).count
    ...
    if existing_count >= MAX_KEYS_PER_ORGANISATION
      errors.add(:base, "Organisation can have a maximum of #{MAX_KEYS_PER_ORGANISATION} #{key_type_name}")
    end
  end
